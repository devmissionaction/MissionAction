...
export default async function NumeroPage({ params }: Params) {
  ...
  const freeArticle = issue.articles?.find((a) => a.type === 'free')
  console.log('Free article:', freeArticle)
  console.log('PDF URL:', freeArticle?.pdf?.asset?.url)

  return (
    <main>
      <section className="min-h-screen flex flex-col items-center justify-center px-8 py-40 max-w-7xl mx-auto">
          ...
          {issue.coverImage?.asset?.url && (
            <div className="flex-shrink-0">
              <img
                ...
                className="w-full md:w-96 max-w-md object-cover rounded-lg shadow-2xl"
              />
            </div>
          )}
          <div className="max-w-xl text-center md:text-left">
            <h1 className="text-5xl md:text-4xl font-bold mb-6">{issue.title}</h1>
            {issue.description && (
      ...
      <section className=" bg-gray-100 flex flex-col items-center justify-center px-8 py-16">
        <h2 className="text-4xl font-bold mb-12 tracking-tight">Accès au contenu</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 max-w-6xl w-full">
          
          <a
            href={freeArticle?.pdf?.asset?.url ?? '#'}
            target="_blank"
